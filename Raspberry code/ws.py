...
def readSensorData(ref, map):
    refv = str(round(read_pin(pot) / 65215 * 1.5, 2))
    mapv = str(round(map_scaling(convert_to_voltage(read_pin(map))), 2))
    return refv, mapv
...
class Senzor(WebSocketClient):
    ...
    def process(self, refv, mapv):
        try:
            tmp = time.localtime()
           # tmp = str(tmp[0]) + '-' + str(tmp[1]) + '-' +  str(tmp[2]) + ":" + str(tmp[3]) + '-' + str(tmp[4]) + '-' +  str(tmp[5])
